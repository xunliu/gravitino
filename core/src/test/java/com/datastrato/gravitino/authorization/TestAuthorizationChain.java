...
public class TestAuthorizationChain {
  ...
  //        chain.newChain(hiveCatalogEntity).chainOps(ChangOps.createRole(),
  // ChangOps.createGroup(1,2,3), ChangOps.createUser(), ...)
  //             .newChain(HdfsCatalogEntity).chainOps(ChangOps.createRole(),
  // ChangOps.createGroup(), ChangOps.createUser());
  //    }
  ...
  void test1() {
    ...
    Function<String, String> function = chain.buildChain(TestAuthorizationOperations::chainTest);

    LOG.info(function.apply("111"));

    Map<String, Function<String, String>> chainMap =
        ImmutableMap.of(
            "chain1", chain.buildChain(TestAuthorizationOperations::chainTest),
            "chain2",
                chain.buildChain(
                    TestAuthorizationOperations::chainTest,
                    TestAuthorizationOperations::chainTest));
    String result = chainMap.getOrDefault("1", Function.identity()).apply("2");
    LOG.info(result);
  }
