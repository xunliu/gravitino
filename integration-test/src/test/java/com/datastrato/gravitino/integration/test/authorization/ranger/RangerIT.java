...
public class RangerIT {
  ...
  public static void setup() {
    ...
    //    String rangerUrl = String.format("http://localhost:%s",
  ...
  protected static String updateOrCreateRangerPolicy(
    ...
    try {
      Thread.sleep(
          30000); // Sleep for a while to wait for the Hive/HDFS Ranger plugin to be updated policy.
  ...
  protected boolean createRangerHiveACL2(
      SecurableObject securableObject, String user, String grouup) {
    RangerPolicy policy = new RangerPolicy();
    policy.setService(RANGER_HIVE_REPO_NAME);
    policy.setName(securableObject.fullName());

    final Splitter DOT_SPLITTER = Splitter.on('.');
    List<String> objects = DOT_SPLITTER.splitToList(securableObject.fullName());
