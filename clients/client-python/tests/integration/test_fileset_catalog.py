...
class TestFilesetCatalog(IntegrationTestEnv):
    ...
    fileset_alter_ident: NameIdentifier = NameIdentifier.of_fileset(metalake_name, catalog_name, schema_name, fileset_alter_name)
    ...
    def setUpClass(cls):
        ...
        # self.gravitino_client.load_catalog(ident=self.catalog_name)
    ...
    def clean_test_data(cls):
        try:
            cls.gravitino_admin_client = GravitinoAdminClient(uri="http://localhost:8090")
            cls.gravitino_client = GravitinoClient(uri="http://localhost:8090", metalake_name=cls.metalake_name)
            # self.metalake = self.gravitino_admin_client.load_metalake(ident=self.metalake_ident)
            cls.catalog = cls.gravitino_client.load_catalog(ident=cls.catalog_ident)
