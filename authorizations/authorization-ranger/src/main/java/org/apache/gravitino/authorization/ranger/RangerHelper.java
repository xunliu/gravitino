...
public class RangerHelper {
  ...
  //    for (int i = 0; i < metadataNames.size() && i < policyResourceDefines.size(); i++) {
  //      searchFilters.put(
  ...
  //      throws AuthorizationPluginException {
  //    List<RangerPolicy> policies = wildcardSearchPolies(authzMetadataObject.names());
  //    if (!policies.isEmpty()) {
  //      /**
  //       * Because Ranger doesn't support the precise search, Ranger will return the policy meets
  // the
  //       * wildcard(*,?) conditions, If you use `db.table` condition to search policy, the Ranger
  // will
  ...
  //    if (policies.size() > 1) {
  //      throw new AuthorizationPluginException("Each metadata object can have at most one
  // policy.");
  //    }
  //
  //    if (policies.isEmpty()) {
