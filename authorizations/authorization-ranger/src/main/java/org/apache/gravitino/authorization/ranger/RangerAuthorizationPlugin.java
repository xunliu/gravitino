...
public abstract class RangerAuthorizationPlugin
  ...
  //  protected abstract void doRenameSchemaMetadataObject(
  //          AuthorizationMetadataObject authzMetadataObject,
  //          AuthorizationMetadataObject newAuthzMetadataObject);
  //  protected abstract void doRenameTableMetadataObject(
  //          AuthorizationMetadataObject authzMetadataObject,
  //          AuthorizationMetadataObject newAuthzMetadataObject);
  //  protected abstract void doRenameColumnMetadataObject(
  //          AuthorizationMetadataObject authzMetadataObject,
  //          AuthorizationMetadataObject newAuthzMetadataObject);
  ...
  private boolean doAddSecurableObject(
    ...
    try {
      ...
      throw new AuthorizationPluginException(
          e, "Failed to add the securable object to the Ranger! %s", e.getMessage());
    }
  ...
  protected void removePolicyByMetadataObject(AuthorizationMetadataObject authzMetadataObject) {
    ...
    //    for (int i = 0;
    //        i < authzMetadataObject.names().size() && i < policyResourceDefinesRule().size();
    //        i++) {
    ...
    //      return policy.getPolicyItems().isEmpty() && policy.getDenyPolicyItems().isEmpty();
    //    }).forEach(policy -> {
    //      try {
    //        rangerClient.deletePolicy(policy.getId());
    //      } catch (RangerServiceException e) {
    //        LOG.error("Failed to delete the policy: {}!", policy, e);
    //      }
    //    });
  }
